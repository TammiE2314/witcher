...
function sortBubbles(list, group){
  const mode = sortMode.value;
  const asc = mode.endsWith("_asc");

  const getDateKey = () => group?.schema?.fields?.find(f=>f.type==="date")?.key || null;
  const getNumberKey = () => group?.schema?.fields?.find(f=>f.type==="number")?.key || null;

  const getTime = (b)=>{
    if(mode.startsWith("created")) return new Date(b.createdAt||0).getTime()||0;
    if(mode.startsWith("updated")) return new Date((b.updatedAt||b.createdAt)||0).getTime()||0;

    if(mode.startsWith("time")){
      const k=getDateKey();
      if(!k) return new Date((b.updatedAt||b.createdAt)||0).getTime()||0;
      const v=b.data?.[k];
    ...
    return new Date((b.updatedAt||b.createdAt)||0).getTime()||0;
  };
